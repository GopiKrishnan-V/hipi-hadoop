...
public class DownloaderInputFormat extends FileInputFormat<IntWritable, Text> 
{
	...
	/**
	 * Assign the equal splits to every node based on the number of lines in the list file and available file
	 * @param job
	 * @return
	 * @throws IOException
	 */
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException 
	{
		...
		int nodes = conf.getInt("downloader.nodes", 10); //Get the number of nodes specified as argument. If no such property it will return 10.
		...
		// Check whether the temporary output directory present
		if (fileSystem.exists(tempOutputDir)) 
		{
			fileSystem.delete(tempOutputDir, true); //Deletes the temporary output directory if present
		}
		fileSystem.mkdirs(tempOutputDir); //Create the temporary output directory if present
